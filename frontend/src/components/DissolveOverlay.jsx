...
function Scene({ imageFrom, imageTo, center, progressRef }) {
  ...
  const uniforms1 = useMemo(() => ({
    ...
    uImageResolution: { value: new THREE.Vector2(texture1.image.width, texture1.image.height) },
    uDissolve: { value: 0.0 },
  ...
  }), [texture1, size, center.x, center.y]);

  const uniforms2 = useMemo(() => ({
    uTexture: { value: texture2 },
    uResolution: { value: new THREE.Vector2(size.width, size.height) },
    uImageResolution: { value: new THREE.Vector2(texture2.image.width, texture2.image.height) },
    uDissolve: { value: 0.0 },
    uCenter: { value: new THREE.Vector2(center.x, center.y) },
    uTime: { value: 0.0 },
